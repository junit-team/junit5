...
abstract class AbstractExtensionContext<T extends TestDescriptor> implements ExtensionContext, AutoCloseable {
	...
	public void publishReportEntry(Map<String, String> values) {
		ReportEntry reportEntry = ReportEntry.from(values);
		String checkpointValue = CheckpointTestDescriptor.getCheckpointValue(values);
		if (StringUtils.isNotBlank(checkpointValue)) {
			executeCheckpoint(CheckpointTestDescriptor.of(this.testDescriptor, checkpointValue));
			//if (values.size() == 1) {
			//	return; // TODO Skip default report entry publication?
			//}
		}
		this.engineExecutionListener.reportingEntryPublished(this.testDescriptor, reportEntry);
	}

	private void executeCheckpoint(CheckpointTestDescriptor checkpoint) {
