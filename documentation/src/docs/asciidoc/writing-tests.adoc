...
* `{TestNameParameterResolver}`: if a method parameter is of type `String` and annotated
  ...
  current test at runtime (either its canonical name or its user-provided `@Name`). This
  acts as a drop-in replacement for the `TestName` rule from JUnit 4. See `MyTest` for an
  example.
