...
* `{TestInfoParameterResolver}`: if a method parameter is of type `{TestInfo}`, the
  ...
  rule from JUnit 4. See `TestInfoDemo` for an example.
...
* `{TestReporterParameterResolver}`: if a method parameter is of type `{TestReporter}`, the
  ...
  be viewed by IDEs or included in reports. In JUnit 5 you would typically use `TestReporter` where
  you used to print information to `stdout`in JUnit 4.
