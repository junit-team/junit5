...
JUnit Jupiter and is analogous to all previous versions of JUnit. Note, however, that the
default test instance lifecycle mode for test classes written in the Kotlin programming
language is "per-class".
...
easier to write non-static `@BeforeAll` and `@AfterAll` methods. Thus, "per-class" mode
is used by default for test classes written in Kotlin. You may switch to "per-method"
semantics by annotating your test class with `@TestInstance(Lifecycle.PER_METHOD)`.
