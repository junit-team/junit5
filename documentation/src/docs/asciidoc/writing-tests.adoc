...
interface default methods. `@BeforeAll` and `@AfterAll` can be declared in static methods of the
test interface while `@ExtendWith` and `@Tag` can be declared in the test interface to configure
extensions and tags. Here's an example:
