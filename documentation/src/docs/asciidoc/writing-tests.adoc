...
* `{TestNameParameterResolver}`: if a method parameter is of type `String` and annotated
  with `@TestName`, the `TestNameParameterResolver` will supply the _display name_ of the
  current test at runtime (either its canonical name or its user-provided `@DisplayName`).
  This acts as a drop-in replacement for the `TestName` rule from JUnit 4. See `MyTest`
  for an example.
...
class MyTest {
	...
	@BeforeEach
	void init(@TestName String name) {
		assertTrue(name.equals("TEST 1") || name.equals("test2"));
	}

	@Test
	@DisplayName("TEST 1")
	void test1(@TestName String name) {
...
* Check out the `methodInjectionTest(...)` test method in
  `{SampleTestCase}` for an example that uses the built-in
  `{TestNameParameterResolver}` as well as two user-provided resolvers,
  `{CustomTypeParameterResolver}` and
