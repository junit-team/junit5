...
The class DisplayNameGenerator already supports the extensions of `ReplaceUnderscores`
and `IndicativeSentencesGenerator`.

By using the annotation that extends `Standard`:
...
With this you can avoid the use of `@DisplayName` and get a prettier output tests results.

Also, by using the annotation that supports the `ReplaceUnderscores`:
...
by using `@DisplayName` or splitting the name of the test with underscores, always
taking `@DisplayName` as a priority, in this way making the results more indicative
and easy to read.

In addition the separator between the names of the tests, can optionally
be customized and if this is not declared, by default the separator `", "`.
Is used. with the annotation:
...
+-- DisplayNameIndicativeSentencesDemo [OK]
  ...
  | +-- A_cat_named_schrodinger >> is in a box [OK]
  | |-- A_cat_named_schrodinger >> can be alive and not at the same time! [OK]
  ' --- A_cat_named_schrodinger >> when we look the box [OK]
      '-- A_cat_named_schrodinger >> when we look the box >> our mind will blow [OK]
```
