...
+-- DisplayNameIndicativeSentencesDemo [OK]
  +-- Schrodingers cat [OK]
  | +-- Schrodinger cat >> is in a box [OK]
  | |-- Schrodinger cat >> can be alive and not at the same time! [OK]
  ' --- Schrodinger cat >> when we look the box [OK]
      '-- Schrodinger cat >> when we look the box >> our mind will blow [OK]
```
...
* `{Alphanumeric}`: sorts test methods _alphanumerically_ based on their names and formal
  parameter lists. **Deprecated in favor of `{MethodName}`
