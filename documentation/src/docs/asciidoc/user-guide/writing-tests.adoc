...
A container or test may be enabled or disabled on a particular operating system or architecture
via the `{EnabledOnOs}` and `{DisabledOnOs}` annotations.
...
`Collection`, `Iterable`, `Iterator`, array, or `Stream` of `DynamicTest` instances. Most
of these examples do not really exhibit dynamic behavior but merely demonstrate the
