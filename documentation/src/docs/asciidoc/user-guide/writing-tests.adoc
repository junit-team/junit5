...
In some cases you can rewrite the logic to use a `CountDownLatch` or any other synchronization mecanism but sometimes it is not possible - generally when depending on an external system.
These tests require some timeout to ensure they don't hang the test suite forever and the test fails if the waiting condition "never" happens.
With `@Timeout` it becomes very easy to write such tests since the test itself just require to become an infinite loop:
...
void waitUntil() {
    while (!isConditionTrue()) {
        sleep(250); // use some adapted retry duration
    }
