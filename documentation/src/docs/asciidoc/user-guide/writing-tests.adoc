...
| `@CleanupMode`           | Used to control how temporary directories are managed. Cleanup modes of ALWAYS, ON_SUCCESS and NEVER can be configured to determine whether a temporary directory is cleaned up or not after a test completes.
|===
...
TempDir has a parameter, cleanup,
which can be set as either NEVER, ON_SUCCESS or ALWAYS. If the cleanup mode is set to NEVER,
temporary directories are not deleted after a test completes. If the cleanup mode is
set to ON_SUCCESS, temporary directories are deleted only after a test completes successfully.

The default cleanup mode for TempDir is ALWAYS. You can use the
...
----
include::{testDir}/example/TempDirCleanupModeDemo.java[tags=user_guide_temp_dir_cleanup_mode]
----
