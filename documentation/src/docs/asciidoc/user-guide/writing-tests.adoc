...
parallel, e.g. to speed up execution, is possible as an opt-in feature since version 5.3.
...
<<writing-tests-parallel-execution-synchronization, synchronization>> mechanisms.
...
`custom`::
  ...
  the `junit.jupiter.execution.parallel.config.custom.class` configuration parameter
...
each other but not while any other test that requires `ReadWrite` access is running.
