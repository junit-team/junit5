...
for an individual test or all tests in a test class. To use it, register the extension and
then annotate a field of type `java.nio.file.Path` or `java.io.File` with `@TempDir` or
add a parameter of type `java.nio.file.Path` or `java.io.File` annotated with `@TempDir`
to a test method, lifecycle method, or test class constructor.

For example, the following test registers the `TempDirectory` extension for a single test
...
The following example registers the `TempDirectory` extension for the entire class and
then stores a _shared_ temporary directory in a `static` field. This allows the same
...
In addition to the default file system for the local operating system, the `TempDirectory`
extension can also be used with any `FileSystem` implementation -- for example,
https://github.com/google/jimfs[Jimfs]. In order to use a custom file system, register the
extension _programmatically_ via `@RegisterExtension` and supply a provider of a custom
...
WARNING: If you configure a `FileSystem` that is not based on the default file system for
the current operating system, declaring a field or parameter of type `File` for your
temporary directory may result in an exception. If that occurs you will need to use `Path`
instead of `File`.
