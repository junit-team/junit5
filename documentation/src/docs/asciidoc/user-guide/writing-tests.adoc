...
an individual test or all tests in a test class. To use it, register the extension and
annotate a field of type `java.nio.file.Path` or `java.io.File` with `@TempDir` or add
a parameter of type `java.nio.file.Path` or `java.io.File` annotated with `@TempDir` to
your test method, lifecycle method, or test class constructor.
...
The following example registers the extension for the entire class and then stores a
_shared_ temporary directory in a `static` field. This allows the same `sharedTempDir` to
be used in all lifecycle methods and test methods of the test class.
