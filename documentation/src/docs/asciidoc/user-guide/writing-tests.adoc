...
Implementations must provide a no-args constructor and should not make any assumptions
regarding when and how many times they are instantiated, but can assume that
`TempDirFactory::createTempDirectory` and `TempDirFactory::close` are both called
once per instance, in this order, and from the same thread.
