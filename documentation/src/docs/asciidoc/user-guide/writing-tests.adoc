...
value is interpreted as a `null` reference. By specifying one or more `nullSymbols`, a
...
| `@CsvSource(value = { "apple, banana, NIL" }, nullSymbols = "NIL")` | `"apple"`, `"banana"`, `null`
|===
...
NOTE: In contrast to the syntax used in `@CsvSource`, `@CsvFileSource` uses a double quote
...
attribute is set; whereas, an entirely _empty_ value is interpreted as a `null` reference.
By specifying one or more `nullSymbols`, a custom value can be interpreted as a `null`
