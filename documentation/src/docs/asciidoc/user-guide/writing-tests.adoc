...
| `@TempDirStrategy`       | Used to control how temporary directories are managed. Cleanup modes of ALWAYS and NEVER can be configured to determine whether a temporary directory is cleaned up or not after a test completes.
|===
...
A test class can be annotated with the TempDirStrategy annotation, which controls how a
class's temporary directories are managed. TempDirStrategy has a parameter, CleanupMode,
which can be set as either NEVER or ALWAYS. If the cleanup mode is set to NEVER,
temporary directories are not deleted after a test completes. TempDirStrategy annotations
may be applied separately to nested test classes.

The default cleanup mode for TempDirStrategy is ALWAYS. You can use the
`junit.jupiter.temp.dir.strategy.cleanup.mode.default` configuration parameter to
override this default.

[source,java,indent=0]
.A test class with a temporary directory that doesn't get cleaned up
----
include::{testDir}/example/TempDirStrategyDemo.java[tags=user_guide_temp_dir_strategy]
----
