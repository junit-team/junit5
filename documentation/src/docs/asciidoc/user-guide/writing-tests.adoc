...
Additionally, when multiple argument sources are provided to a `@ParameterizedTest`,
`{ArgumentsAccessor}` makes the test iteration number available to the test using
the `{getInvocationIndex()}` method.
