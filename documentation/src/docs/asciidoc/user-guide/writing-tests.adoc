...
like regular test classes but annotated with the `{ParameterizedClass}` annotation.
...
field injection. If a `{Parameter}`-annotated field is declared in the parameterized
