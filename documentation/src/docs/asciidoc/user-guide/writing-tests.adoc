...
[[override-default-display-name-generator]]
==== Global configuration to override the default DisplayNameGenerator
Along with using `@DisplayName` and `@DisplayNameGeneration` annotation for custom display name
, it is possible to change the default display name generator for
the execution of an entire test plan. To change the default display name generator,
set the `junit.jupiter.displayname.generator.default` configuration parameter to the fully qualified
name of an class implementing `org.junit.jupiter.api.DisplayNameGenerator`.
The configuration can be supplied as a JVM system property, as a _configuration parameter_ in the
...
For example, to use `DisplayNameGenerator.ReplaceUnderscores` as a default display name generator
you can start your JVM with the following system property.

`-Djunit.jupiter.displayname.generator.default=org.junit.jupiter.api.DisplayNameGenerator.DisplayNameGenerator.ReplaceUnderscores`

make sure you use fully qualifies name (with class with package name) for the class and the class should
implements the `org.junit.jupiter.api.DisplayNameGenerator` interface.

precedence of display name will be
1. `@DisplayName` annotation
2. Global jupiter configuration parameter `junit.jupiter.displayname.generator.default`
3. `@DisplayNameGeneration` annotation
