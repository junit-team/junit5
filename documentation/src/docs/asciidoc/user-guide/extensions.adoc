...
[[extensions-test-instance-pre-create-callback]]
=== Test Instance Pre-create Callback

`{TestInstancePreCreateCallback}` defines the API for `Extensions` that wish to be invoked
_prior_ to test instances being created (by a constructor call or via a factory).
