...
extension APIs such as `BeforeAllCallback`, `AfterAllCallback`, `TestInstancePostProcessor`,
 and `TestInstancePreDestroyCallback` as well as method-level extension APIs such as
...
Common use cases include cleaning dependencies that have been injecting into the
