...
<2> Retrieve the HTTP server instance from the session-level store
...
----
<1> Get the host string directly from the injected HTTP server instance
<2> Get the port number directly from the injected HTTP server instance
