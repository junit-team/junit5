...
Similarly, the parallelization won't work if we only enable the parallel execution,
but we don't specifically enable a parallel execution level:
...
In such cases, all the tests will be executed sequentially:
[source,plaintext]
----
main - BarTest::test1
main - BarTest::test2
