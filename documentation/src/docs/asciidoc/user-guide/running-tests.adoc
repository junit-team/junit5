...
WARNING: IntelliJ IDEA (prior to IDEA 2017.3) bundles a certain version of JUnit 5. Thus,
...
Tagging by test type (_micro_, _integration_, _end-to-end_) and feature (*foo*, *bar*, *baz*) the following tag expressions can be useful.

[cols="40,60"]
|===
|tag expression                             | selects
...
| +foo & !end-to-end+                       | all tests for *foo*, but not the _end-to-end_ tests
| +(micro \| integration) & (foo \| baz )+  | all _micro_ or _integration_ tests for *foo* or *baz*
|===
