...
for executing tests on the JUnit Platform. To enable it, you just need to add one line to
your `build.gradle`:
...
The standard `test` tasks does currently not provide a dedicated DSL to set configuration
parameters to influence test discovery and execution. However, you can use system
properties instead.
...
`{LogManager}`, set the `java.util.logging.manager` system property to the supplied _fully
...
NOTE: Please use Surefire 2.21.0 with `junit-platform-surefire-provider`.
