...
1. The `configurationParameter()` and `configurationParameters()` methods in the
  ...
  * <<running-tests-console-launcher,Console Launcher>>: use the `--config` or `--config-resource`
    command-line option.
...
2. The `configurationParametersResources()`
  method in the `LauncherDiscoveryRequestBuilder` which is used to select a `.properties`
...
precedence over those supplied via system properties, custom properties files,
and the platform configuration file.
