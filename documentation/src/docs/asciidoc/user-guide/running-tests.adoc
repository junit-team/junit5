...
The following pattern styling should be used for following

- <<extensions-conditions-deactivation>>
- <<launcher-api-listeners-custom-deactivation>>

If the pattern _configuration parameter_ consists solely of an asterisk
(`+*+`), all candidate classes will be matched. Otherwise, the pattern will be used to match
against _comma separated_ fully qualified class name (_FQCN_) of each candidate class. Any dot
...
- `+*.MyCustom+`: matches every candidate class whose simple class name is
  exactly `MyCustom`.
- `+*System*+`: matches every candidate class whose simple class name contains
  `System`.
- `+*System*+, +*Unit*+`: matches every candidate class whose simple class name
...
- `org.example.MyCustomImpl, org.example.TheirCustomImpl`: matches both the candidate classes
   whose FQCN is exactly `org.example.MyCustomImpl` and `org.example.TheirCustomImpl`.
