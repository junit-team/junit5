...
The `pom.xml` file in the `{junit5-jupiter-starter-maven}` project demonstrates how to
use it and can serve as a starting point.

The JUnit team has developed a basic provider for Maven Surefire that lets you run JUnit 4
and JUnit Jupiter tests via `mvn test`. With the release of Surefire 2.22.0 this basic
provider will deprecated and discontinued soon.
