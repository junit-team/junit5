...
With the release of Surefire 2.22.0 the `junit-platform-surefire-provider` from the JUnit
team will be deprecated and discontinued soon thereafter.
...
When the https://github.com/remkop/picocli[picocli] command line parser encounters an argument
beginning with the character `@`, it expands the contents of that file into the argument list.
...
This will likely result in an "unmatched argument" error message.
You can troubleshoot by executing the command with system property `picocli.trace` set to `DEBUG`.
...
You can still pass a real parameter with an initial '@' character by escaping it
