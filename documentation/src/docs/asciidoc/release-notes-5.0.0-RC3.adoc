...
refer to the link:index.html#running-tests-ide-intellij-idea[instructions] in
the user manual to use this version in a version of IntelliJ IDEA that bundles
an older milestone or release candidate.
...
** via a file named `junit-platform.properties` in the root of the class path. See
   link:index.html#running-tests-config-params[running-tests-config-params] for details.
** via the `--config` command-line option when using the
   link:index.html#running-tests-console-launcher[Console Launcher].
** via the `configurationParameter` or `configurationParameters` DSL when using the
   link:index.html#running-tests-build-gradle-config-params[Gradle plugin].
** via the `configurationParameters` property when using the
   link:index.html#running-tests-build-maven-config-params[Maven Surefire provider].
...
* The _default_ test instance lifecycle mode can now be set via a _configuration
  ...
  See link:index.html#writing-tests-test-instance-lifecycle-changing-default[writing-tests-test-instance-lifecycle-changing-default] for details.
