...
An executable `junit-platform-console-{platform-version}-standalone.jar` with all dependencies
included is provided at the central Maven repository under the
https://repo1.maven.org/maven2/org/junit/platform/junit-platform-console[junit-platform-console]
directory. You can https://docs.oracle.com/javase/tutorial/deployment/jar/run.html[run] the
application like shown below:

`java -jar junit-platform-console-{platform-version}-standalone.jar <<<running-tests-console-launcher-options>>>`
