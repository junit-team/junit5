...
or execute JUnit 5 tests with a <<running-tests-junit-platform-runner,JUnit 4 based Runner>>.
...
buildscript {
	...
	dependencies {
		classpath 'org.junit:junit-platform-gradle-plugin:{junit-version}'
	}
}
...
```groovy
dependencies {
	testCompile("org.junit:junit-jupiter-api:5.0.0-SNAPSHOT")
	testRuntime("org.junit:junit-jupiter-engine:5.0.0-SNAPSHOT")
}
```
...
dependencies {
	...
	testRuntime("org.junit:junit-vintage-engine:5.0.0-SNAPSHOT")
}
```
...
Test failures (1):
  JUnit 5:SecondTest:mySecondTest()
...
for example, a JUnit 5 test class.
...
* `junit-jupiter-engine` in _test runtime_ scope: implementation of the Engine API for JUnit 5
