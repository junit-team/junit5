...
use a newer milestone version of the Jupiter API, executing the tests might not work.
This situation will improve once the first GA version of JUnit 5 has been released.
In the meantime, please follow the instructions below to use a newer version of JUnit 5
than the one bundled with IntelliJ IDEA.
...
Eclipse offers full support for JUnit 5 as a part of their Oxygen.1a (4.7.1a) release.

For more information on using JUnit 5 in Eclipse check https://www.eclipse.org/eclipse/news/4.7.1a/#junit-5-support[Eclipse Project Oxygen.1a (4.7.1a) - New and Noteworthy.]

==== Other IDEs

At the time of this writing, there is no direct support for running tests on the JUnit
Platform within IDEs other than with IntelliJ IDEA and Eclipse.
