...
* Stack traces produced by failing tests are now pruned of calls from JUnit and Java by
  default. This feature can be disabled via the `junit.platform.stacktrace.pruning.enabled`
  configuration parameter, or configured to exclude other fully qualified class names
  by providing a list of patterns to `junit.platform.stacktrace.pruning.pattern`.
