...
** via a file named `junit-platform.properties` in the root of the class path. See
   <<../user-guide/index.adoc#running-tests-config-params,Configuration Parameters>> for details.
** via the `--config` command-line option when using the
   <<../user-guide/index.adoc#running-tests-console-launcher,Console Launcher>>.
** via the `configurationParameter` or `configurationParameters` DSL when using the
   <<../user-guide/index.adoc#running-tests-build-gradle-config-params,Gradle plugin>>.
** via the `configurationParameters` property when using the
   <<../user-guide/index.adoc#running-tests-build-maven-config-params,Maven Surefire provider>>.
...
* The _default_ test instance lifecycle mode can now be set via a _configuration
  ...
  See <<../user-guide/index.adoc#writing-tests-test-instance-lifecycle-changing-default,Changing the Default Test Instance Lifecycle>> for details.
