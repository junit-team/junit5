...
* Filters for class name, engine IDs, and tags in the Gradle plugin configuration now need to be
  wrapped in a `filters` extension element (see <<../user-guide/index.adoc#running-tests-build-gradle-filters,Configuring Filters>>).
...
* The Gradle plugin now allows to explicitly select which tests should be executed
  (see <<../user-guide/index.adoc#running-tests-build-gradle-selectors,Configuring Selectors>>).
...
* Multiple regular expressions that are combined using OR semantics can now be passed to the JUnit
  Platform Gradle plugin (see <<../user-guide/index.adoc#running-tests-build-gradle-filters,Configuring Filters>>) and the `{ConsoleLauncher}`
...
* Package names can now be included or excluded via a filter configuration to the JUnit Platform
  Gradle plugin (see <<../user-guide/index.adoc#running-tests-build-gradle-filters,Configuring Filters>>) and the `{ConsoleLauncher}`
  (see <<../user-guide/index.adoc#running-tests-console-launcher-options,Options>>).
...
* Support for lazy and preemptive _timeouts_ with lambda expressions in `Assertions`. See
  examples in <<../user-guide/index.adoc#writing-tests-assertions,`AssertionsDemo`>> and consult the
