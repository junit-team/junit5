...
* New Kotlin friendly `fail` methods added as _top-level functions_ in the
  `org.junit.jupiter.api` package. When calling the original methods from Kotlin the compiler
  required the generic return type of `fail` to be declared explicitly calling it.
** Eg: `fail<Nothing>("Some message")`. These new methods all return
