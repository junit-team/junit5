...
* Dynamic tests now require less memory thanks to a number of improvements to internal
  data structures.
* In parameterized test with `@MethodSource` or `@ArgumentSource`, arguments can now have optional names. When the argument is included in the display name of an iteration, this name will be used instead of the value.
