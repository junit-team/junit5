...
* New `junit-platform-reporting` artifact containing an `XmlReportsWritingListener` that
  generates XML reports using a format which is compatible with the de facto standard for
  JUnit 4 based test reports that was made popular by the Ant build system.
