...
* `PackageSource`, `ClassSource`, and `MethodSource` do no longer implement the `JavaSource`
   interface, which has been removed, but implement the `TestSource` interface directly.
