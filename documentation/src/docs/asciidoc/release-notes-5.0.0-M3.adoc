...
* Filters for class name, engine IDs, and tags in the Gradle plugin configuration now need to be
  wrapped in a `filters` extension element (see link:index.html#running-tests-build-gradle-filters[running-tests-build-gradle-filters]).
...
* The Gradle plugin now allows to explicitly select which tests should be executed
  (see link:index.html#running-tests-build-gradle-selectors[running-tests-build-gradle-selectors]).
...
* Multiple regular expressions that are combined using OR semantics can now be passed to the JUnit
  Platform Gradle plugin (see link:index.html#running-tests-build-gradle-filters[running-tests-build-gradle-filters]) and the `{ConsoleLauncher}`
...
* Package names can now be included or excluded via a filter configuration to the JUnit Platform
  Gradle plugin (see link:index.html#running-tests-build-gradle-filters[running-tests-build-gradle-filters]) and the `{ConsoleLauncher}`
  (see link:index.html#running-tests-console-launcher-options[running-tests-console-launcher-options]).
...
* Support for lazy and preemptive _timeouts_ with lambda expressions in `Assertions`. See
  examples in link:index.html#writing-tests-assertions[`AssertionsDemo`] and consult the
