...
public class NestedStaticClassResolver extends JUnit5TestResolver {
	private static final Logger LOG = Logger.getLogger(NestedStaticClassResolver.class.getName());
		...
		/*
			...
			private List<TestDescriptor> getTestDescriptorsForTestClasses(TestDescriptor parent, List<Class<?>> testClasses) {
				...
				for (Class<?> testClass : testClasses) {
					if (ReflectionUtils.isStatic(testClass)) {
