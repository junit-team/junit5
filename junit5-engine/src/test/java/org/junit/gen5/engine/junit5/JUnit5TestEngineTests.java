...
public class JUnit5TestEngineTests {
	...
	@org.junit.Test
	public void executeTestsFromClasses() {
		...
		Assert.assertEquals("# descriptors", 6, descriptors.size());

		TrackingTestExecutionListener listener = new TrackingTestExecutionListener();
		engine.execute(new TestExecutionContext(descriptors, listener));
	...
	public void executeTestFromUniqueId() {
		...
		List<TestDescriptor> descriptors = engine.discoverTests(spec);
		Assert.assertNotNull(descriptors);
		Assert.assertEquals("# tests", 4, descriptors.size());
