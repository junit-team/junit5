...
public class JavaTestDescriptor implements TestDescriptor {
  ...
  public static JavaTestDescriptor from(final String uid) throws Exception {
    ...
    Preconditions.condition(matcher.matches(), () -> String.format("Invalid format for %s UID: %s",
        JavaTestDescriptor.class.getSimpleName(), uid));
  ...
  private static String createTestId(Class<?> testClass, Method testMethod) {
    return (testMethod != null ? String.format("%s#%s(%s)", testClass.getName(),
        testMethod.getName(), nullSafeToString(testMethod.getParameterTypes()))
        : testClass.getName());
  }
