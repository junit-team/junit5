...
class TheoryDisplayNameFormatterTests {
    ...
    public void testEndToEnd() throws Exception {
        ...
        TheoryInvocationContext invocationContext = new TheoryInvocationContext(currentPermutationIndex, theoryParameterArguments, null,
        ...
        InputExpectedResultValuePair inputAndExpectedResult = buildInputAndExpectedResulStrings(
                ...
                new InputExpectedResultValuePair(Theory.CURRENT_PERMUTATION_PLACEHOLDER, Integer.toString(currentPermutationIndex + 1)),
                new InputExpectedResultValuePair(Theory.TOTAL_PERMUTATIONS_PLACEHOLDER, Integer.toString(totalPermutations)),
                new InputExpectedResultValuePair(Theory.PARAMETER_VALUES_PLACEHOLDER,
                        theoryParameterArguments.entrySet().stream()
                                .map(v -> (String) v.getValue().getValue())
                                .collect(joining(", "))),
                new InputExpectedResultValuePair(Theory.PARAMETER_VALUES_WITH_INDEXES_PLACEHOLDER,
                        theoryParameterArguments.entrySet().stream()
                                .map(v -> v.getValue().getValue() + " (index " + v.getKey() + ")")
                                .collect(joining(", "))),
                new InputExpectedResultValuePair(Theory.PARAMETER_DETAILS_PLACEHOLDER, invocationContext.getArgumentsDescription(", ")));

        //Test
        TheoryDisplayNameFormatter formatterUnderTest = new TheoryDisplayNameFormatter(inputAndExpectedResult.input, displayName, totalPermutations);
    ...
    private InputExpectedResultValuePair buildInputAndExpectedResulStrings(InputExpectedResultValuePair... entries) {
        String input = Stream.of(entries)
                .map(v -> v.input)
                .collect(joining("] [", "[", "]"));
        String expectedResult = Stream.of(entries)
                .map(v -> v.expectedResult)
                .collect(joining("] [", "[", "]"));
