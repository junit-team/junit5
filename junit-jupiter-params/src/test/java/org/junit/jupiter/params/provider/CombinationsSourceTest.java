...
class CombinationsSourceTest {
    ...
    static Stream<Arguments> stringSourceMethod() {
        return CombinationsSource.fromSources(
                Collections.singleton("one"), Collections.singleton("two"), Collections.singleton("three"));
    }
    ...
    void MultiArgsCheck() {
        ...
        Stream<Arguments> arguments = CombinationsSource.fromSources(
        ...
        for (int eachArguments = 0; eachArguments < (1 << 8); eachArguments++) {
            ...
            for (int argIdx = 0; argIdx < 8; argIdx++) {
                ...
                assertThat(arg)
                        .describedAs("arguments " + eachArguments + " values=" + Arrays.toString(values))
                        .isEqualTo(((1 << argIdx) & eachArguments) != 0);
            }
        }
    }
    ...
    void fromPrimitiveArrays() {
        Stream<Arguments> arguments = CombinationsSource.fromSources(
                ...
                PRIMITIVE_FLOAT, PRIMITIVE_INT, PRIMITIVE_LONG, PRIMITIVE_DOUBLE);
    ...
    void fromStreams() {
        Stream<Arguments> arguments = CombinationsSource.fromSources(
                IntStream.of(PRIMITIVE_INT), LongStream.of(PRIMITIVE_LONG),
                DoubleStream.of(PRIMITIVE_DOUBLE), Stream.of(REF_ARRAY));
    ...
    void fromBoxedArrays() {
        Stream<Arguments> arguments = CombinationsSource.fromSources(
                BOXED_BOOLEAN, BOXED_BYTE, BOXED_CHAR, BOXED_SHORT,
    ...
    void fromManySources() {
        Stream<Arguments> arguments = CombinationsSource.fromSources(
                boolean.class, Boolean.class, AnEnum.class, ITERATOR, ENUMERATION, ITERABLE, REF_ARRAY,
                PRIMITIVE_BOOLEAN, PRIMITIVE_BYTE, PRIMITIVE_CHAR, PRIMITIVE_SHORT,
                PRIMITIVE_FLOAT, PRIMITIVE_INT, PRIMITIVE_LONG, PRIMITIVE_DOUBLE,
                BOXED_BOOLEAN, BOXED_BYTE, BOXED_CHAR, BOXED_SHORT,
                BOXED_INT, BOXED_FLOAT, BOXED_LONG, BOXED_DOUBLE);
    ...
    void emptySource() {
        assertThrowsExactly(IllegalArgumentException.class, () -> {
            Stream<Arguments> arguments = CombinationsSource.fromSources(
                    Collections.emptySet(), Collections.emptyList(), Collections.emptyIterator());
    ...
    void testSplitting() {
        ...
        needsSplit.push(new CombinationsSource.ArraysSpliterator(true, 3L,
                CombinationsSource.convertSources(
                        true, boolean.class, Boolean.class, boolean.class, Boolean.class,
                boolean.class, Boolean.class, boolean.class, Boolean.class )));
        List<Spliterator<Object[]>> doneSplits = new ArrayList<>();
        do {
            Spliterator<Object[]> didSplit = needsSplit.peek().trySplit();
            if (null != didSplit) {
        ...
        Object[][] collected = doneSplits.stream()
                .flatMap(aSplit -> StreamSupport.stream(aSplit, false))
                .toArray(Object[][]::new);
        ...
        for (int eachArgument = 0; eachArgument < (1 << 8); eachArgument++) {
            ...
            for (int argIdx = 0; argIdx < 8; argIdx++) {
                ...
                boolean arg = (Boolean) values[7 - argIdx];
                assertThat(arg)
                        .describedAs("arguments " + eachArgument + " values=" + Arrays.toString(values))
                        .isEqualTo(((1 << argIdx) & eachArgument) != 0);
            }
        }
    }
    ...
    void singleSplit() {
        Deque<Spliterator<Object[]>> needsSplit = new ArrayDeque<>();
        needsSplit.push(
                new CombinationsSource.ArraysSpliterator(true, 3L, IntStream.range(0, 256).boxed().toArray()));
        List<Spliterator<Object[]>> doneSplits = new ArrayList<>();
        do {
            Spliterator<Object[]> didSplit = needsSplit.peek().trySplit();
            if (null != didSplit) {
        ...
        int[] collected = doneSplits.stream()
                .flatMap(aSplit -> StreamSupport.stream(aSplit, false))
                .mapToInt(args -> (Integer) args[0])
                .toArray();
        assertThat(collected).hasSize(1 << 8);
